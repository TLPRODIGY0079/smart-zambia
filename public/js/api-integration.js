...
async function loadDestinationsFromAPI() {
  try {
    ...
    const cached = window.CacheManager.get('destinations');
    if (cached && cached.length > 0) {
      console.log(`Showing ${cached.length} cached destinations`);
      window.destinations = cached;
      renderDestinations();
      
      // Check if cache is stale (older than 5 minutes)
      if (!window.CacheManager.isStale('destinations', 300000)) {
      ...
      if (typeof window.showDestinationsSkeleton === 'function') {
        window.showDestinationsSkeleton();
      }
    }
    ...
    // Cache the results
    window.CacheManager.set('destinations', destinations, 3600000); // 1 hour
    
    // Re-render destinations with fresh data
    ...
    if (typeof window.showToast === 'function') {
      window.showToast('Failed to load destinations. Showing cached data.', 'warning');
    } else {
      console.warn('Failed to load destinations. Showing cached data.');
    }
